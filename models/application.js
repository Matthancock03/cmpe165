...
q.post('update', function(application){
    if(application.signed) {
        Job.findOne({_id: application.jobId}, function (job) {
            if (job.applicantIdsToSign.contains(application.ownerId)) {//Check to see that the user has been sent the request
                ...
                email.save();//Don't do this on the frontend. different behavior. would create an object.
                job.save();//ditto.
            }
        });
    }
