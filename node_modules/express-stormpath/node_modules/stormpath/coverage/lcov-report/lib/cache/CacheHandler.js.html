...
<div class="header high">
    ...
    <h2>
        ...
        Statements: <span class="metric">90.77% <small>(59 / 65)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Branches: <span class="metric">72.73% <small>(24 / 33)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
...
&nbsp;
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (href) {
    region = href.match(/customData/) ? <span class="branch-0 cbranch-no" title="branch not covered" >'customData' </span>: (href.split('/').slice(-2)[0]);
  }
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!region || CACHE_REGIONS.indexOf(region) === -1) {
<span class="cstat-no" title="statement not covered" >    return disabledCache;</span>
  }
&nbsp;
...
function buildCacheableResourcesFromParentObject(object){
  ...
  var parentResource = {};
  <span class="missing-if-branch" title="if path not taken" >I</span>if(utils.isCollectionData(object)){
<span class="cstat-no" title="statement not covered" >    object.items.forEach(<span class="fstat-no" title="function not covered" >function(resource) {</span></span>
<span class="cstat-no" title="statement not covered" >      Array.prototype.push.apply(resourcesToCache, buildCacheableResourcesFromParentObject(resource));</span>
    });
  }else{
    _.pairs(object).forEach(function(pair) {
      var prop = pair[0];
      var val = pair[1];
      <span class="missing-if-branch" title="if path not taken" >I</span>if(val &amp;&amp; val.href &amp;&amp; _.keys(val).length&gt;1){
<span class="cstat-no" title="statement not covered" >        Array.prototype.push.apply(resourcesToCache, buildCacheableResourcesFromParentObject(val));</span>
<span class="cstat-no" title="statement not covered" >        parentResource[prop] = {</span>
          href: val.href
        };
      }else{
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if(parentResource.href){
...
CacheHandler.prototype.put = function cacheResource(href, data, _new, cb) {
  var _this = this;
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (typeof _new === 'function') {
    cb = _new;
    _new = true;
  }
&nbsp;
